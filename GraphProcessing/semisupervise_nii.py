...
if __name__ == "__main__":
    ...
    for i in range(0, max(set(labels))+1):
        buf_index_array = np.array(np.where(labels == i))
        for j in buf_index_array[0]:
            buf_volume_array[np.where(label_int_data == j)] = i + 1
        print("Cluster {} in {} has {} labels, and it has been saved.".format(i, n_clusters_, len(buf_index_array[0])))
    ...
    print("Segmentation results have been saved in nii.gz file.")
