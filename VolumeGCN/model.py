...
class Volume_GCN:
    def __init__(self, args, G, initial_feature_vector):
        self.hp = args['hp']
        self.G = G
        self.initial_feature_vector = initial_feature_vector
        # self.f, self.w1, self.w2, self.W = get_initialization(self.hp, self.G)
        self.f = get_initialization(self.hp, self.initial_feature_vector)

    def conv(self, A):
        with tf.variable_scope("GCN", reuse=tf.AUTO_REUSE) as scope:
            hidden_features = self.f

            for i in range(len(self.hp.hidden_layers)):
                hidden_features = tf.matmul(A, hidden_features)
                hidden_features = tf.layers.dense(hidden_features, self.hp.hidden_layers[i],
                                activation=tf.nn.relu, name="hidden_layer_{}".format(i), reuse=tf.AUTO_REUSE)
                hidden_features = tf.layers.dropout(hidden_features, self.hp.dropout, name="dropout_layer_{}".format(i))
    ...
    def predict(self, A, xu):
        ...
        xs_emb = tf.squeeze(tf.nn.embedding_lookup(hidden_features, xu))
        logits = tf.layers.dense(xs_emb, self.hp.label, activation=None, name="classifer", reuse=tf.AUTO_REUSE)

        pre = tf.argmax(logits, 1)
        return pre

    def predict_old(self, A, xu):
        h_1 = tf.matmul(A, self.f)
        h_1 = tf.layers.dense(h_1, self.hp.hidden1, activation=tf.nn.relu, name="hidden_layer_1", reuse=tf.AUTO_REUSE)
