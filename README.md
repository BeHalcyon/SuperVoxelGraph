...
**STEP 3.1 Calculating graph for histogram using python(Histogram2Graph)**

The Histogram2Graph transfers the two "txt" file to gexf graph using networkx package.

Input informaiton:
```c
*.json: the configure file including edge_weight_array.txt and label_histogram_array.txt, and the unlabeled gexf file path.
```
...
**STEP 3.2 Calculating labeled graph from the unlabled graph in STEP 3.1**

Input informaiton:
```c
*.json: the configure file including the unlabeled and labeled gexf file path, and some csv files generated by gephi.
...
Output information:
```c
*_supergraph_labeled.gexf: labeled graph file for the volume data.
```

**STEP 4. Training the graph using GCN in an unsupervised manner and supervised manner** 

Document organization:
```c
unspervise_GCN.py: training GCN for unlabeled graph.
...
```cpp
{
    ...
    "gcn":{
      ...
      "ratio": 0.8// The proportion of training set
    }
  }
```
